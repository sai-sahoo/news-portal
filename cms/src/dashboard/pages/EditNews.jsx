...
const EditNews = () => {
	...
	const get_edit_news = async () => {
		try {
			const { data } = await axios.get(`${base_url}/api/news/edit/${news_id}`, {
				headers: {
					Authorization: `Bearer ${store.token}`,
				},
			});
			setContentLang(data?.news?.contentLang);
			setCategoryId(data?.news?.categoryId);
			setTitle(data?.news?.title);
			setTitleRegional(data?.news?.titleRegional);
			setDescription(data?.news?.description);
			setMetaTitle(data?.news?.metaTitle);
			setMetaDescription(data?.news?.metaDescription);
			setMetaKeywords(data?.news?.metaKeywords);
			const imageUrl = data?.news?.image;
			setImg(imageUrl && imageUrl !== "null" ? imageUrl : null);
  ...
  <div className="min-h-screen bg-gray-100 p-8">
    <div className="max-w-5xl mx-auto bg-white shadow-md rounded-lg border border-gray-200">
      ...
      <form onSubmit={updateNews} className="px-8 py-8 space-y-8">
